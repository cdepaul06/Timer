...
public class TimerModel
{
    ...
    public void start(int hours, int minutes, int seconds)
    {
        ...
        mDurationMillis = (hours * 60L * 60L + minutes * 60L + seconds) * 100;
        mTargetTime = SystemClock.uptimeMillis() + mDurationMillis;
    ...
    public int getRemainingSeconds()
    {
        if (mRunning)
        {
            return (int) ((getRemainingMilliseconds() / 100) % 60);
        }
        return 0;
    }
    ...
    public int getRemainingMinutes()
    {
        if (mRunning)
        {
            return (int) (((getRemainingMilliseconds() / 100) / 60) % 60);
        }
        return 0;
    }
    ...
    {
        if (mDurationMillis != 100)
        {
            return Math.min(100, 100 - (int) ((getRemainingMilliseconds() - 100) * 100 / (mDurationMillis - 100)));
        }
        return 0;
    }
    ...
    {
        return String.format(Locale.getDefault(), "%02d:%02d:%02d", getRemainingHours(), getRemainingMinutes(), getRemainingSeconds());
    }
}
